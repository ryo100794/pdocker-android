...
def main() -> int:
    ...
    require("vulkan icd lets transfer-only submits complete and forwards compute", "has_dispatch" in vulkan_icd_src and "queue-submit transfer-only command buffer" in vulkan_icd_src and "send_generic_vulkan_dispatch" in vulkan_icd_src and "VULKAN_DISPATCH_V1" in vulkan_icd_src)
    ...
    require("direct runtime rewrites unix socket paths", "rewrite_unix_sockaddr_arg" in direct_src and "AF_UNIX" in direct_src and "case 203" in direct_src and "ADD_TRACE_SYSCALL(203)" in direct_src and "/run/pdocker-gpu" in pdockerd_bridge_src)
    ...
    require("project cards prefer engine id labels over name prefix", "projectContainerSnapshots(projectDir: File)" in main_src and "labelledProject == projectId" in main_src and "composeNames" in main_src)
    require("project service health is gated by engine container state", "projectServiceHealthSummary(serviceUrls, dir)" in main_src and "snapshots.none { containerSnapshotIsRunning(it) }" in main_src)
