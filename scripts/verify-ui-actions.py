...
def main() -> int:
    ...
    require("readonly log terminals use compact font", "const initialFontSize = readOnly ? 10 : 12" in xterm_src and "let fontSize = initialFontSize" in xterm_src)
    ...
    require("container exposes explicit engine api it terminal", "action_container_terminal_fmt" in string_src and "detail_container_terminal" in string_src and "ENGINE_EXEC_PREFIX" in main_src and "/exec/$execId/start" in bridge_src_now and "docker exec -it" not in main_src)
    ...
    require("interactive terminal font remains 12pt", "const initialFontSize = readOnly ? 10 : 12" in xterm_src and "fontSize: initialFontSize" in xterm_src)
