...
# --- jniLibs sanity ---
# libpdockerpty.so, libpdockerdirect.so, and libpdockergpuexecutor.so are built
...
# nativeLibraryDir.
for lib in libpdockerpty.so libpdockerdirect.so libpdockergpuexecutor.so libpdockergpushim.so; do
